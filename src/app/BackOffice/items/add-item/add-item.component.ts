...
export class AddItemComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.categoryService.getAllCategories().subscribe(
      (response) => {
        this.categoryTypes = response as any;
      },
      (error) => {
  ...
  onSubmit() {
    if (this.itemForm.valid) {
      ...
      this.ajouterImage(() => {
        ...
        this.itemService.addItem(newItem, categoryId).subscribe(
          (response) => {
            console.log('Item added successfully:', response);
            this.router.navigate(['/dashboard/listItems']); // Navigate back to the item list
          },
          (error) => {
