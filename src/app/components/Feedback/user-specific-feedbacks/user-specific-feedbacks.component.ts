...
import { AvatarService } from 'src/app/services/FeedbackService/avatar.service';
...
import { Reaction, Reacts } from 'src/app/services/reacts';
import { UserService } from 'src/app/services/user.service';

@Component({
  selector: 'app-user-specific-feedbacks',
  templateUrl: './user-specific-feedbacks.component.html',
  styleUrls: ['./user-specific-feedbacks.component.css']
})
export class UserSpecificFeedbacksComponent {
  ...
  userId: string | null = '';
  activePicker: number | null = null;
  reactionAnimation: string | null = null;
  ...
  loadUserFeedbacks(): void {
    ...
    this.feedbackService.getFeedbacksByUserId(parseInt(this.userId)).subscribe({
      next: (data: Feedback[]) => {
        console.log('Raw feedback data:', data); // Debug log
        
        this.feedbacks = data.map(fb => {
          // Formater correctement l'avatar
          const avatarUrl = fb.userAvatar 
            ? (fb.userAvatar.startsWith('data:image') 
                ? fb.userAvatar 
                : `data:image/*;base64,${fb.userAvatar}`)
          ...
          return {
            ...
            user: {
              ...
              name: fb.userName,
              email: fb.userEmail,
              avatar: avatarUrl // URL formatée
            },
            userAvatar: avatarUrl // Compatibilité ascendante
          };
        });
  
        // Debug: Vérifier les avatars après mapping
        console.log('Processed feedbacks with avatars:', 
      ...
      error: (error) => {
        ...
        this.errorMessage = 'Failed to load your feedbacks';
      }
    });
  }
  // All other methods from your original component can be copied as-is
  // (getUserReaction, addReaction, getReactionEmoji, etc.)
  // ...

  // Make sure to copy all the helper methods from your original component
  // that you need for the template functionality

  // The only difference is that we're filtering feedbacks by user ID
  // in the initial load (loadUserFeedbacks method)
